using Microsoft.EntityFrameworkCore.Metadata.Internal;
using System.ComponentModel.DataAnnotations;
using System.ComponentModel.DataAnnotations.Schema;

namespace SistemPendataanJemaat.Models.Entities
{
    [Table("tbl_status_keaktifan")]
    public class StatusKeaktifanEntityModel
    {
        public string Status_Keaktifan_ID { get; set; }

        [Display(Name = "Status Keaktifan")]
        [Required(ErrorMessage = "Status Keaktifan is required")]
        [StringLength(100, ErrorMessage = "Status Keaktifan can't be longer than 100 characters")]
        public string Status_Keaktifan { get; set; }

        
        [Display(Name = "Keterangan")]
        [StringLength(200, ErrorMessage = "Keterangan can't be longer than 200 characters")]
        public string Keterangan { get; set; }
    }
}
